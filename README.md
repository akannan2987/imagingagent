...
4. **Agent-operable.** Every capability is exposed as a validated, read-only
   ...
   author.
5. **Self-explaining.** From Phase 3b, `imagingagent ask "…"` answers
   questions about the project from its own documentation — with the
   passages and links — and refuses questions that are not about it
   rather than guessing.
6. **Traceable.** Every run writes a line to an append-only ledger with the
...
| pathology | DeepLIIF (validation set) | IHC images with pixel-aligned multiplex immunofluorescence of the same tissue | IHC positivity, paired-stain multi-modal check | CC-BY 4.0 |
| pathology | MCMICRO exemplar-001 | one multiplex-immunofluorescence tissue core, 12 channels in 3 cycles (raw, unregistered tiles) | mIF segmentation, per-channel positivity, cycle registration | see data doc |
...
| 0c — Two-track refactor | track config, `VolumeGeometry` | track config, five dataset roles, `TileGeometry` | `--track` on every command, modality registry, lock-based CI, figures as scripts, 53 tests | ✅ built |
| 1 — Ingestion | MSD download script, synthetic volumes + DICOM series, NIfTI/DICOM readers with geometry | five download scripts (Kather-2016, PanNuke fold, DeepLIIF, MCMICRO, Visium), readers for TIFF/PNG, OME-TIFF, composites, Parquet, h5ad; three synthetic generators | case manifest, `ingest` / `manifest` commands, shared download helper with checksums, track CI job | ✅ built |
| 2 — Preprocessing | resample, normalise, denoise, register | stain separation, normalisation, tissue mask, tiling; MCMICRO cycle registration | augmentation | ⏳ next |
| 3 — Segmentation | classical + 3D U-Net, surface metrics | classical + InstanSeg, AJI / PQ | import any label map · **v0.1.0** | ⏳ |
| 3b — Docs assistant | — | — | `imagingagent ask`: retrieval over the documentation with citations and an out-of-scope gate; provider layer for grounded answers | ⏳ |
...
| 1c | The Phase 1 tutorial with by-hand walkthroughs of every reader, every download with expected output, and the ingestion-flow figure | [`docs/04-phase-tutorials/01-ingestion.md`](docs/04-phase-tutorials/01-ingestion.md) |
| 1b | Pathology readers (tiles, DeepLIIF composites, OME-TIFF, PanNuke, Visium); three synthetic generators; ingestion for five dataset roles; PanNuke Parquet extractor; five download scripts on a shared helper with checksums and publisher MD5s; `requirements-pathology.txt`; CI installs both tracks; 77 tests | [`docs/04-phase-tutorials/01-ingestion.md`](docs/04-phase-tutorials/01-ingestion.md) |
| 1a | Shared manifest; mri NIfTI/DICOM readers preserving geometry; synthetic volumes and DICOM series; MSD download with checksums; `ingest` and `manifest` commands; `requirements-mri.txt`; CI job for track extras | [`docs/04-phase-tutorials/01-ingestion.md`](docs/04-phase-tutorials/01-ingestion.md) |
| 0c | `tracks:` configuration; `VolumeGeometry` / `TileGeometry` contracts with a consistency validator; modality registry (implemented and planned); `--track` on `doctor`, `modalities`, `ledger list`; new `tracks` and `modalities` commands; per-track extras; CI installing from `requirements.lock`; every figure generated by `scripts/figures/`; 53 tests | [`docs/04-phase-tutorials/00c-two-track-refactor.md`](docs/04-phase-tutorials/00c-two-track-refactor.md) |
...
│   └── mri_only.yaml                  example: one track switched off
...
│   ├── schemas.py                     data contracts; VolumeGeometry | TileGeometry
│   ├── modality.py                    modality registry — implemented and planned
│   ├── manifest.py                    the case manifest — the pipeline's shopping list
│   ├── tracks/mri/                    io.py (NIfTI/DICOM + geometry) · synth.py · ingest.py
│   └── tracks/pathology/              io.py (tiles, OME-TIFF, composites, h5ad) · synth.py · pannuke.py · ingest.py
│   ├── ledger.py                      append-only run history
│   ├── cli.py                         the command line
│   └── utils.py                       hashing, timestamps, identifiers
├── scripts/_download.py               shared: resumable download, checksums, extraction
├── scripts/download_*.py              one per dataset: msd_hippocampus, kather2016, pannuke, deepliif, mcmicro_exemplar, visium_sample
├── scripts/figures/                   one script per illustration; all.py regenerates every one
├── requirements-mri.txt               pinned extras for the mri track
├── requirements-pathology.txt         pinned extras for the pathology track
├── requirements.lock                  the frozen dependency tree CI installs from
...
└── docs/
    ...
    └── img/                           cover, architecture, geometry, audit, branch, setup, datasets, config and ingestion illustrations
```

Later phases add `src/imagingagent/tracks/mri/`, `tracks/pathology/`,
`audit/`, `benchmark/`, `report/`, `serve/`, `scripts/download_*.py`,
`scripts/synth_*.py` and one tutorial per phase; each is added to this map
when it lands.
...
imagingagent tracks                   # both tracks, their datasets and settings
imagingagent modalities               # every modality, implemented or planned
python -m pip install -r requirements-mri.txt -r requirements-pathology.txt   # once: both tracks' readers
imagingagent ingest --track mri --synthetic --limit 5          # no download needed
imagingagent ingest --track pathology --synthetic --limit 8    # all five dataset roles, synthetic
imagingagent manifest --track pathology  # the cases, their geometry and roles
pytest                                # 77 passed
```
...
```bash
imagingagent ingest   --track mri        # built · --track pathology built
