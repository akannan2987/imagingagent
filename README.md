...
| 1 — Ingestion | ✅ MSD download script, synthetic volumes + DICOM series, NIfTI/DICOM readers with geometry | ⏳ five public datasets, three synthetic generators | ✅ case manifest, `ingest` / `manifest` commands, track CI job | 🔧 in progress |
...
├── scripts/download_msd_hippocampus.py   MSD Task04 download with checksum recording
...
python -m pip install -r requirements-mri.txt      # once: the mri track's readers
imagingagent ingest --track mri --synthetic --limit 5   # no download needed
imagingagent manifest --track mri     # the cases and their geometry
pytest                                # 65 passed
```
...
```bash
imagingagent ingest   --track mri        # built · --track pathology arrives with Phase 1b
