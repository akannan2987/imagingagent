...
| 1b | Pathology readers (tiles, DeepLIIF composites, OME-TIFF, PanNuke, Visium); three synthetic generators; ingestion for five dataset roles; PanNuke Parquet extractor; five download scripts on a shared helper with checksums and publisher MD5s; `requirements-pathology.txt`; CI installs both tracks; 77 tests | tutorial arrives with part 1c |
| 1a | Shared manifest; mri NIfTI/DICOM readers preserving geometry; synthetic volumes and DICOM series; MSD download with checksums; `ingest` and `manifest` commands; `requirements-mri.txt`; CI job for track extras; 65 tests | tutorial arrives with part 1c |
