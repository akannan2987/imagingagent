...
`result       : healthy`, and `pytest` ends with `53 passed`.
...
**A note on illustrations.** Static documents on GitHub can show pictures
and Mermaid diagrams; they cannot be interactive. Genuinely interactive
figures arrive twice later: the Streamlit review interface (Stage 15) and
interactive HTML plots saved beside the benchmark report (Stage 14). Every
static figure is generated by a script in `scripts/figures/`.

---

## Stage 4 — Two-track refactor (Phase 0c) ✅

**What.** Follow [`04-phase-tutorials/00c-two-track-refactor.md`](04-phase-tutorials/00c-two-track-refactor.md).
...
**Also in this stage:** the requirements lock file that CI now installs
from ([ADR 0007](adr/0007-lock-file-and-per-track-dependencies.md)), and
every illustration becoming a script under `scripts/figures/` — including
the ones this handbook shows.

![Branch model](img/git_branch_model.svg)

**Checkpoint.** `imagingagent doctor` lists optional libraries under
*[mri]*, *[pathology]* and *[serve]* headings; `imagingagent modalities`
shows implemented and planned modalities; `pytest` → `53 passed`.
**Commit message:** `phase-0c: two-track configuration, geometry contracts, modality registry, --track on every command, figures as scripts, lock-based CI`.
