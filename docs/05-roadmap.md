...
| 3b | Docs assistant *(shared)* | — | — | `imagingagent ask`: retrieval over README, Handbook, glossary, architecture, tutorials, ADRs, roadmaps and docstrings; passages returned verbatim with file + heading links; a confidence gate that refuses questions outside the project instead of guessing; optional grounded answers through the language-model provider layer (pulled forward from Phase 11) with three backends — template (no model), local (an open model served by Ollama on Windows/macOS/Linux), hosted (any provider with an OpenAI-compatible HTTP interface) — chosen by two environment variables; starter questions shown when the box is empty and "you might also ask" suggestions generated from the index; a golden question set (in-scope and out-of-scope) enforced in CI | < 1 min | ⏳ | — |
...
| Docs assistant, embedding-based retrieval (small local embedding model on CPU) | ⏳ Phase 3b, optional | word-based retrieval (BM25) ships first; the embedding option switches on once PyTorch is installed in Phase 3 and improves matching for questions phrased differently from the documents |
